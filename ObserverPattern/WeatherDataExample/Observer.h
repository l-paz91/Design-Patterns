...
//--INCLUDES--//
#include "std_lib_facilities.h" // found in my p&p repo
...
{
public:
	virtual void update(float pTemp, float pHumidity, float pPressure) = 0;
};
...
public:
	...
	void update(float pTemp, float pHumidity, float pPressure)
	{
		mTemperature = pTemp;
		mHumidity = pHumidity;
		display();
...
public:
	...
	void update(float pTemp, float pHumidity, float pPressure)
	{
		mHeatIndex = computeHeatIndex(pTemp, pHumidity);
		display();
	}
	// End Observer implementation
